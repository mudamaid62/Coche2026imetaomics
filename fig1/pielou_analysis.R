...
pdf("pielou_analysis.pdf")
beta_metadata <- read_tsv("blas_metadata.tsv")

pielou_mx <- read_tsv("pielou_matrix") %>% 
  column_to_rownames(var = "ESCG") %>% t()

set.seed(91297)
distancias_smx <- as.matrix(vegdist(pielou_mx, method = "bray"))
...
# pielou_mx: your Sites x 43 genes matrix
# ecotypes: your vector of ecotype labels
posthoc_results <- pairwise_mv_posthoc(pielou_mx, ecotype, score = "r", stand = "o", method = "p")
print(posthoc_results)

# 1. Calculate the mean Pielou index across all 43 genes for each site
pielou_summary <- data.frame(
  Avg_Pielou = rowMeans(pielou_mx),
...
ggbetweenstats(
  ...
  p.adjust.method = "fdr",
  centrality.plotting = TRUE,
  title = "Multivariate Pielou Index Comparison",
  ylab = "Mean Pielou Index (43 genes)",
  xlab = "Ecotype",
  point.args = list(position = ggplot2::position_jitterdodge(dodge.width = 0.6), alpha = 1, size = 3, stroke = 0, na.rm = TRUE),
  violin.args = list(width = 0.5, linewidth = 0.2, alpha = 0.2, na.rm = TRUE),
  boxplot.args = list(width = 0.3, alpha = 0.2, na.rm = TRUE),
