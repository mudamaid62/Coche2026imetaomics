...
pdf("richness_analysis.pdf")
beta_metadata <- read_tsv("blas_metadata.tsv")

richness_mx <- read_tsv("richness_matrix") %>% 
  column_to_rownames(var = "ESCG") %>% t()

set.seed(91297)
distancias_smx <- as.matrix(vegdist(richness_mx, method = "bray"))
...
# richness_mx: your Sites x 43 genes matrix
# ecotypes: your vector of ecotype labels
posthoc_results <- pairwise_mv_posthoc(richness_mx, ecotype, score = "r", stand = "o", method = "p")
print(posthoc_results)

# 1. Calculate the mean Richness index across all 43 genes for each site
richness_summary <- data.frame(
  Avg_Richness = rowMeans(richness_mx),
...
ggbetweenstats(
  ...
  p.adjust.method = "fdr",
  centrality.plotting = TRUE,
  title = "Multivariate Richness Comparison",
  ylab = "Mean Richness (43 genes)",
  xlab = "Ecotype",
  point.args = list(position = ggplot2::position_jitterdodge(dodge.width = 0.6), alpha = 1, size = 3, stroke = 0, na.rm = TRUE),
  violin.args = list(width = 0.5, linewidth = 0.2, alpha = 0.2, na.rm = TRUE),
  boxplot.args = list(width = 0.3, alpha = 0.2, na.rm = TRUE),
